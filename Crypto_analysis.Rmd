...
# comment out if already install 
install.packages(c(
  'tidyverse', 'quantmod', 'xts', 'zoo', 'forecast',
  'tseries', 'urca', 'PerformanceAnalytics', 'rugarch',
  'ggplot2', 'lubridate'
))
```
...
```{r Cryptocurrency Analysis , echo = FALSE }
crypto_symbol <- "BTC"  
```
...
```{r 30-Day Rolling Volatility , echo = FALSE }
...
```{r GARCH Model for Volatility , echo = FALSE}
...
```{r Conditional Volatility , echo = FALSE }
plot(garch_fit, which = 3)

```

## Volatility Forecast
```{r Volatility Forecast, echo = FALSE }
...
```{r Relative Strength Index (RSI) , echo = FALSE}
...
```{r Seasonality Analysis Optional , echo = FALSE}
...
# 7. Saving Results
## run the code in R so can save the result and data from yahoo finance. 
```{echo = FALSE}
# Create directory for results
if (!dir.exists("crypto_analysis_results")) {
  dir.create("crypto_analysis_results")
}

# Save cleaned data
write.csv(crypto_df, file.path("crypto_analysis_results", 
                              paste0(tolower(crypto_symbol), "_data_2018_present.csv")), 
          row.names = FALSE)

# Save workspace
save.image(file.path("crypto_analysis_results", 
                    paste0(tolower(crypto_symbol), "_analysis.RData")))

cat("Analysis complete for", crypto_symbol, "!\n")
cat("Results saved in 'crypto_analysis_results' folder.\n")
